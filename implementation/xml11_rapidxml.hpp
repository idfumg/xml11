...
void Parse(NodeImpl& root, rapidxml::xml_node<>* node)
{
    for (auto n = node->first_attribute(); n; n = n->next_attribute()) {
        NodeImpl new_node {n->name(), n->value()};
...
std::shared_ptr<NodeImpl> ParseXml(std::string text)
{
    ...
    for (const auto& node : nodeImpl->nodes()) {
        ...
        case Node::Type::ATTRIBUTE:
            new_attribute = doc.allocate_attribute(
                node->name().c_str(),
                doc.allocate_string(node->text().c_str()));
...
std::string ToXml(const std::shared_ptr<NodeImpl>& root, const bool indent)
{
    ...
    try {
        ...
        xml_node<>* root_node = doc.allocate_node(node_element, root->name().c_str());
